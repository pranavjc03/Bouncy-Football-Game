...
float goalkeeper_dy = 0.001; // Slower goalkeeper movement

// Size of the goalkeeper
...
void drawBackground() {
    glColor3f(0.0, 0.8, 0.0); // Green color for the football field
    glBegin(GL_QUADS);
    glVertex2f(-1.0, -1.0);
    glVertex2f(1.0, -1.0);
    glVertex2f(1.0, 1.0);
...
void drawBall() {
    glColor3f(1.0, 1.0, 1.0); // White color for the ball
    glBegin(GL_POLYGON);
    for (int i = 0; i < 360; i++) {
...
// Function to draw the goalpost
void drawGoalpost() {
    glColor3f(0.5, 0.5, 0.5); // Gray color for the goalpost

    // Draw the solid part of the goalpost
    glBegin(GL_QUADS);
    ...
    // Draw the checkered lines on the goalpost
...
void updateScore() {
    if (ball_x + ball_radius > -1.0 - goalpost_width && ball_x - ball_radius < -1.0 && ball_y + ball_radius < goalpost_height / 2 && ball_y - ball_radius > -goalpost_height / 2) {
        score++;
        std::stringstream ss;
        ss << "Score: " << score;
        scoreText = ss.str();
...
int main(int argc, char** argv) {
    ...
    glClearColor(0.0, 0.5, 0.0, 1.0); // Green color for the background
...
// Function to draw the goalkeeper
void drawGoalkeeper() {
    // Draw the head of the goalkeeper as a small semicircle
...
void idle() {
    if (gameState == RUNNING) {
        ...
        // Goalkeeper wall detection
        if (goalkeeper_y + goalkeeper_height / 2 > 1.0 || goalkeeper_y - goalkeeper_height / 2 < -goalpost_height / 2) {
            goalkeeper_dy *= -1;
        }
    }
